...
class ThtCinemaWebsiteBooking(http.Controller):
    ...
    def cnm_web_momo_result(self, order=None, access_token=None, **kw):
        ...
        if kw.get('resultCode', '') == "0" :
            ...
            dbv_obj.write({'state': 'done', 'payment_method': 'momo', 
                'user_id': request.session.uid ,
                'date_hoadon': datetime.datetime.now().strftime(DEFAULT_SERVER_DATE_FORMAT),
                })
    ...
    def cnm_soatve_result(self, **kw):
        ...
        if order_name != '':
            dbv_obj = request.env['dm.donbanve'].sudo().search([('name','=',order_name)], limit=1)
            if dbv_obj: 
                values = {
                    'dbv_obj': dbv_obj
                }
    ...
    @http.route(['/cnm/web/momo/notify'], type='http', auth="public", methods=['POST'], csrf=False, website=True, cors='*')
