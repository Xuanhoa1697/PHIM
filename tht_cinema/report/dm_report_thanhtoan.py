...
class DmReportThanhToan(models.Model):
    _name = 'dm.report.thanhtoan'
    _description = 'Report Thanh Toan'
    ...
    detail_lines = fields.One2many('dm.report.thanhtoan.detail', 'report_id', string='Dữ liệu chi tiết')
    general_lines = fields.One2many('dm.report.thanhtoan.general', 'report_id', string='Dữ liệu tổng hợp')
    ...
    def action_general(self):
        ...
        for data in self:
            ...
            if data.dm_ptthanhtoan_id:
                domain += [('dm_ptthanhtoan_id','=',data.dm_ptthanhtoan_id.id)]
            # if d1:
            ...
            for order in orders:
                for line in order.dm_donbanve_line_ids:
                    ...
                    if 1 == 1 :
                        ...
                        merger_code_detail = str(order.dm_ptthanhtoan_id.ht_thanhtoan)
                        ...
                        if merger_code_detail not in data_detail:
                            data_detail[merger_code_detail] = {
                                ...
                                'ht_thanhtoan': order.dm_ptthanhtoan_id.ht_thanhtoan,
            ...
            for k,v in data_detail.items():
                ...
                self.env['dm.report.thanhtoan.detail'].create({
                    'dm_lichchieu_id': v['dm_lichchieu_id'],
                    'ngay': v['suatchieu'],
                    'dm_phim_id': v['dm_phim_id'],
                    'ht_thanhtoan': v['ht_thanhtoan'],
                    'tenphim': v['tenphim'],
            ...
            for kg,vg in data_general.items():
                ...
                self.env['dm.report.thanhtoan.general'].create({
...
class DmReportThanhToanDetail(models.Model):
    _name = 'dm.report.thanhtoan.detail'
    _description = 'dm.report.thanhtoan.detail'
    ...
    report_id = fields.Many2one('dm.report.thanhtoan', string='Báo cáo bán vé', ondelete='cascade')
    dm_ptthanhtoan_id = fields.Many2one('dm.ptthanhtoan', string='Phương thức thanh toán')
...
class DmReportthanhtoanGeneral(models.Model):
    _name = 'dm.report.thanhtoan.general'
    _description = 'dm.report.thanhtoan.general'
