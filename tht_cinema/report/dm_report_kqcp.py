...
class DmReportKqcp(models.Model):
    _name = 'dm.report.kqcp'
    _description = 'Report KQCP'
    ...
    detail_lines = fields.One2many('dm.report.kqcp.detail', 'report_id', string='Dữ liệu chi tiết')
    general_lines = fields.One2many('dm.report.kqcp.general', 'report_id', string='Dữ liệu tổng hợp')
    ...
    #     return super(DmReportKqcp, self).create(vals)
    ...
    #     return super(DmReportKqcp, self).write(vals)
    ...
    def action_general(self):
        ...
        for data in self:
            ...
            if not data.dm_phim_id: 
                dm_lichchieu = self.env['dm.lichchieu'].search([('batdau','>=',d1),('batdau','<',d2),('sudung',"=",True)])
            else : 
                dm_lichchieu = self.env['dm.lichchieu'].search([('batdau','>=',d1),('batdau','<',d2),('sudung',"=",True), ('dm_phim_id','=',data.dm_phim_id.id)])
            ...
            if dm_lichchieu:
                ...
                for rec in dm_lichchieu:
                    ...
                    lc_info = {
                        'id': rec.id, 
                        'dm_lichchieu_id': rec.id,
                        'ngay': rec.ngaychieu,

                        'dm_phim_id': rec.dm_phim_id.id,
                        'tenphim': rec.dm_phim_id.name,
                        'nhaphathanh': rec.dm_phim_id.nhaphathanh,
                        'phong': rec.dm_phong_id.name,
                        'soghe': rec.dm_phong_id.tongsoghe,
                        'tongsove': 0,
                        'suatchieu': rec.batdau,
                        'product_id': 0,
                        'qty': 0,
                        'price_unit': 0,
                        'total_amount': 0,
                    }
                    if rec.dm_phim_id.id not in lc_ids_obj:
                        lc_ids_obj[rec.dm_phim_id.id] = {rec.id: lc_info}
                    else:
                        lc_ids_obj[rec.dm_phim_id.id].update({rec.id: lc_info})
            ...
            for k,v in data_detail.items():
                key_dm_phim_id = v['dm_phim_id']
                key_lc_id = v['dm_lichchieu_id']
                if key_lc_id in lc_ids_obj[key_dm_phim_id]:
                    del lc_ids_obj[key_dm_phim_id][key_lc_id]

                self.env['dm.report.kqcp.detail'].create({
            ...
            for kg,vg in data_general.items():
                self.env['dm.report.kqcp.general'].create({
...
class DmReportKqcp(models.Model):
    _name = 'dm.report.kqcp.detail'
    _description = 'dm.report.kqcp.detail'
    ...
    report_id = fields.Many2one('dm.report.kqcp', string='Báo cáo bán vé', ondelete='cascade')
    
class DmReportKqcpGeneral(models.Model):
    _name = 'dm.report.kqcp.general'
    _description = 'dm.report.kqcp.general'
