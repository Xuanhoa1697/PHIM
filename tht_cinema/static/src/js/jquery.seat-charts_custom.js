			...
			if (settings.naming.left) {
				$row.append(
					$('<div></div>')
						...
						.addClass('seatCharts-cell row-name')
			...
			$.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) { 
				...
				$row.append(character != '_' ?
					...
					(function(naming) {
						...
						console.log('overrideLabel:', overrideLabel)
						
						// ajin-custom
						if (settings.seats[character].classes =='s-class'){
							space = $('<div></div>').addClass('seatCharts-cell seatCharts-space s-class')
							overrideLabel = naming.rows[row] + naming.columns[column] + " " + naming.rows[row] + (parseInt(naming.columns[column]) + parseInt(1))
							overrideId = naming.rows[row] + naming.columns[column] + "_" + naming.rows[row] + (parseInt(naming.columns[column]) + parseInt(1))
							// if (naming.columns[column][0]< naming.columns[column][1]){
							// 	space = $('<div></div>').addClass('seatCharts-cell seatCharts-space-s-class')
							// 	overrideLabel = naming.rows[row] + naming.columns[column] + " " + naming.rows[row] + (parseInt(naming.columns[column]) + parseInt(1))
							// 	overrideId = naming.rows[row] + naming.columns[column] + "_" + naming.rows[row] + (parseInt(naming.columns[column]) + parseInt(1))
							// } 
							...
							space = $('<div></div>').addClass('seatCharts-cell seatCharts-space')	
							// space = 'aa'
