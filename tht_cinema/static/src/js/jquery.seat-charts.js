	...
	$.fn.seatCharts = function (setup) {
		...
		$.each(settings.map, function(row, characters) {
			...
			if (settings.naming.left) {
				$row.append(
					$('<div></div>')
						.addClass('seatCharts-cell seatCharts-space row-name')
			...
			$.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) { 
				...
				if (settings.naming.columns[column] == '' || settings.naming.rows[row] == '' ){

					$row.append(
						$('<div></div>').addClass('seatCharts-cell seatCharts-space loidi')	
					);
